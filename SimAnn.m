function Params=SimAnn(InputParams,BB,H,TC) 
% This function runs the simulated annealing based fitting process on input
% data stored in matrices 'BB', 'H', and 'TC'. The number of dimensions of 
% parameter space is set by the number of rows in input matrix 'InputParams'. 
% The boundaries of the parameter space along the different dimensions are 
...
% measure of goodness of fit (in our case, the root sum of squared deviations 
% between data points and the reference model, a.k.a. the root mean square 
% error or RMSE) within the boundaries of the parameter space. To do this, we 
...
% 'Data' folder, respectively. For a description of the data matrices, see 
% the headers of the corresponding data files.
%
...
% root sum of squared deviations between data points and the reference model, 
% a.k.a. the root mean square error or RMSE) corresponding to the best fit 
% parameter values stored in the previous vector elements.
%
%
% Credits: 
...
% with it.
OutStat=Blooming(Params,BB,H,TC);
T_start=OutStat;
...
while(T>T_crit)
    ...
    if((Test_Params(ParamInd)>=InputParams(ParamInd,1))&(Test_Params(ParamInd)<=InputParams(ParamInd,2)))            
        Test_OutStat=Blooming(Test_Params,BB,H,TC);       
    else        
